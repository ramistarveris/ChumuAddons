...
const renderTerm = ({ text, x, y, z }) => {
    ...
    if (config.termLabel) {
        ...
        if (label && distanceToPlayer(x, y, z) < config.termLabelClassDistance) {
...
register("renderWorld", () => {
    if (!config.showTerm) return;
    const phase = Data.goldorsection;
    if (!phases[phase]) return;
    phases[phase].forEach(renderTerm);
});
